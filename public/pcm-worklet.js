/**
 * AudioWorklet processor: captures mic input, downmixes to mono, resamples to
 * `targetSampleRate` (linear interpolation) and posts Int16 PCM chunks of
 * ~`chunkMs` to the main thread as transferable ArrayBuffers.
 *
 * On a `{type:"flush"}` port message it posts the partial tail chunk followed
 * by `{type:"flushed"}`, then stops processing.
 *
 * KEEP IN SYNC with PCM_PROCESSOR_SOURCE in client/src/worklet.ts.
 */
